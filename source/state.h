...
// When adding a new state, note to also add it to GetStateName - that how the
// logger gets the name of the state
enum HomemenuState {
	STATE_NONE = 0,		  // State hasn't been initialized
	STATE_FOREGROUND = 1, // Currently in the foreground, user can make actions
	STATE_WAIT_TO_REGISTER =
		2,				  // In transition to new app, user can still do actions
	STATE_BACKGROUND = 3, // In background, must still handle shutdowns
	STATE_STARTING_POMELO = 4, // When starting pomelo
	STATE_CLOSING_APP = 5,	   // When closing the currently open app
};
...
void GetStateName(enum HomemenuState state, char *nameOut) {
	...
	case STATE_BACKGROUND:
		...
		return;
	case STATE_STARTING_POMELO:
		strcpy(nameOut, "Starting Pomelo");
		return;
	case STATE_CLOSING_APP :
		strcpy(nameOut, "Closing Current App");
		return;
	default:
...
enum HomemenuState GetState() { return state; }
...
void SetState(enum HomemenuState newState) {
	...
	log_debug("Changing state from \"%s\" (0x%x) to \"%s\" (0x%x)",
			  oldStateName, state, newStateName, newState);
